...
const appMiddlewareInventario = Router();
const appDTOData = Router();
const appValidateData = express();

appValidateData.use(async (req, res, next) => {
  try {
    const data = plainToClass(Inventario, req.body, { excludeExtraneousValues: true });
...
appMiddlewareInventario.use((req, res, next) => {
...
appDTOData.use(async (req, res, next) => {
  try {
    let data = plainToClass(DTO('inventarios').class, req.body);
    await validate(data);
